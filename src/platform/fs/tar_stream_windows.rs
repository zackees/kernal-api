...
// correct NTFS link flavor per target and falling back to copies after any
// creation or verification failure, preserving the compatibility behavior.
// ---------------------------------------------------------------------------
    ...
    while !pending.is_empty() {
        ...
        for link in &unresolved {
            ...
            let Some(_) = resolve_link_target(dest, &link_path, &link.target) else {
                // Reject lexically absolute/escaping targets. This does not
                // inspect existing reparse points beneath the destination;
                // it is not a filesystem containment guarantee.
                eprintln!(
...
fn materialize(
    ...
    match created.and_then(|()| verify_materialization(&link_path, is_dir)) {
        ...
        Err(err) => {
            ...
            // to a copy of the lexically-contained target. Filesystem links
            // may still redirect that target; this requires a trusted tree.
...
use crate::platform::fs::resolve_archive_link_target as resolve_link_target;
