//! macOS child-control, standard-stream, and jobserver mechanics.
//!
//! The neutral contracts live in `crate::platform::process`; this file only
//! answers them for macOS.
...
/// macOS offers the GNU make `R,W` descriptor-pair jobserver.
...
impl NativeJobserver {
    ...
    pub fn create(capacity: usize) -> io::Result<Self> {
        ...
        // SAFETY: `pipe` succeeded, so both descriptors are fresh and owned.
        let (read, write) = unsafe { (OwnedFd::from_raw_fd(fds[0]), OwnedFd::from_raw_fd(fds[1])) };
        // macOS has no `pipe2`; mark both ends close-on-exec as Linux does.
