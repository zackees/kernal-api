//! macOS local IPC transport mechanics.
...
pub fn legacy_send_fd_over(
    ...
    let sent = unsafe { libc::sendmsg(socket_fd, &message, libc::MSG_DONTWAIT) };
    if sent < 0 {
...
impl InheritedListener {
    ...
    pub fn recover_from_env(env_key: &str) -> io::Result<Option<Listener>> {
        ...
        if !is_listening_socket(fd)? {
            return Err(io::Error::new(io::ErrorKind::InvalidInput, format!("{env_key}={fd} does not name a listening socket")));
        }
        use interprocess::os::unix::uds_local_socket::Listener as UdsListener;
        use std::os::fd::{FromRawFd as _, OwnedFd};
        // SAFETY: validated inherited descriptor in a fresh child table.
...
fn parse_descriptor(env_key: &str, raw: &str) -> io::Result<i32> {
    let fd: i32 = raw.trim().parse().map_err(|_| io::Error::new(io::ErrorKind::InvalidInput, format!("{env_key}={raw:?} is not a descriptor number")))?;
    if fd < 0 { return Err(io::Error::new(io::ErrorKind::InvalidInput, format!("{env_key}={fd} is not a valid descriptor"))); }
    Ok(fd)
}
...
fn clear_cloexec(fd: &std::os::fd::BorrowedFd<'_>) -> io::Result<()> {
    ...
    if unsafe { libc::fcntl(raw, libc::F_SETFD, flags & !libc::FD_CLOEXEC) } < 0 { return Err(io::Error::last_os_error()); }
    Ok(())
}
...
fn socket_option(fd: i32, option: libc::c_int) -> io::Result<libc::c_int> {
    ...
    if unsafe { libc::getsockopt(fd, libc::SOL_SOCKET, option, std::ptr::addr_of_mut!(value).cast(), std::ptr::addr_of_mut!(len)) } < 0 { return Err(io::Error::last_os_error()); }
    Ok(value)
}

fn is_listening_socket(fd: i32) -> io::Result<bool> {
    if socket_option(fd, libc::SO_TYPE)? != libc::SOCK_STREAM { return Ok(false); }
...
mod legacy_handoff_tests {
    ...
    fn saturated_ancillary_queue_emsgsize_maps_to_silent_would_block() {
        let error = std::io::Error::from_raw_os_error(libc::EMSGSIZE);
        assert_eq!(
            legacy_send_error_kind(&error),
            HandoffTransferErrorKind::WouldBlock
        );
