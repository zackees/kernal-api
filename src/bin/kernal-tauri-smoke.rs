...
impl SmokeScenario {
    fn from_args() -> Result<Self, Box<dyn std::error::Error>> {
        match std::env::args().nth(1).as_deref() {
            ...
            Some(other) => Err(format!(
                "unknown smoke scenario {other:?}; use close, popup, redirect, timeout, cancel, window-close, bootstrap, or offscreen-webgl"
            )
            .into()),
        }
    }
    ...
    fn page(self, address: &str) -> String {
        let action = match self {
            Self::Close | Self::Bootstrap | Self::OffscreenWebgl => "",
