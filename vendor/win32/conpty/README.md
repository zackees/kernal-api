...
Release CI packages only the committed LFS objects and rejects bytes that do
not reproduce the committed archive manifest. **The refresh process
...
6. Regenerate `conpty-sidecar.sha256.toml` with
   `ci/build_conpty_assets.sh`, then open a PR.
