//! macOS filesystem-change-notification backend, adapted from the `notify`
//! crate's `FSEvents` backend.
//!
//! `notify`'s macOS backend carries no divergence from the published
...
/// `notify` backend (FSEvents on macOS).
...
///
/// `notify::Event::need_rescan` is how FSEvents' `MustScanSubDirs` (the
/// kernel could not keep up and coalesced changes into "rescan this
/// subtree") surfaces once it reaches the public API; the watch itself
/// keeps running, so this is never a lost watch on macOS.
...
mod tests {
    ...
    /// fixed sleep because FSEvents delivery timing and coalescing are not
    /// guaranteed.
    #[test]
    fn watching_a_directory_reports_a_file_creation() {
        let directory = tempfile::tempdir().expect("temp directory");
        // FSEvents reports paths under the root's canonical spelling (macOS
        // temp directories are usually a symlink, e.g. `/var` -> `/private/var`),
        // so watch and compare against the canonicalized root rather than the
        // one `tempfile` handed back, or the paths will never match.
        let root = std::fs::canonicalize(directory.path()).expect("canonicalize temp directory");
