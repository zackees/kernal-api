//! Retirement using held file handles rather than reopening ambient paths.
use std::fs::File;
use std::io;

/// No upgrade needed on Unix: an open file can be unlinked directly.
#[cfg(unix)]
pub fn open_for_retire(file: File) -> io::Result<File> {
    Ok(file)
}

/// Retire `file`: drop the handle and run the caller's remove. Unix
/// unlinks a mapped image immediately, so the plain remove is the whole
/// contract.
#[cfg(unix)]
pub fn retire_open_file(
    file: File,
    plain_remove: impl FnOnce() -> io::Result<()>,
) -> io::Result<()> {
    drop(file);
    plain_remove()
}
...
pub fn open_for_retire(file: File) -> io::Result<File> {
    ...
    // SAFETY: the input File owns a live handle throughout ReOpenFile.
    let handle = unsafe {
        ReOpenFile(
