use super::cgroup_v2_dir;
use std::path::Path;

const PROC_MEMINFO: &str = "/proc/meminfo";

/// One best-effort, portable observation of the resource boundary containing
/// this process. Cgroup fields are `None` off Linux or when the controller is not
/// readable; an unreadable value is never interpreted as zero.
///
/// Values describe the observed cgroup, not the minimum of ancestor limits.
/// Counter files are read separately and may change between reads. Explicit
/// `read_at` paths can be used for fixtures on any host.
...
pub struct HostResourceSnapshot {
    ...
    /// Tasks (including threads) charged to the cgroup (`pids.current`).
...
impl HostResourceSnapshot {
    ...
    /// Return this cgroup's finite memory headroom, saturating at zero.
    /// Ancestor constraints may impose a smaller effective budget.
