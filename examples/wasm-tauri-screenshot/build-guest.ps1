...
try {
    ...
    function Test-ScreenshotTargetLibraries {
        $libdir = (& soldr rustc --print target-libdir --target $target).Trim()
    ...
    & soldr cargo build --locked --manifest-path Cargo.toml --target $target --release --target-dir $guestTargetDirectory @guestFeatures
