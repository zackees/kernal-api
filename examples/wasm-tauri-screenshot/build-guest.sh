...
# Use the same pinned threaded build lane, through Soldr and its cache, as
# the existing real Rust smoke artifact. Never substitute a hand-authored Wasm fixture.
(
  cd "$example_dir/guest"
  guest_target_materialized() {
    local libdir
    libdir="$(soldr rustc --print target-libdir --target "$target")" || exit "$?"
  ...
  if ! guest_target_materialized; then
    ...
    if ! guest_target_materialized; then
      guest_sysroot="$(soldr rustc --print sysroot)"
