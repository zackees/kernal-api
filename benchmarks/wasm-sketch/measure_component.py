...
def validate_output(output: str, module_bytes: int) -> None:
    expected = [f"validated component: {module_bytes} bytes; one kernel import; not executed",
                "Wasmtime 45 component compilation passed; not instantiated or executed"]
    if output.splitlines() != expected:
        raise ValueError("encoder must validate and engine-compile this exact component without execution")
...
def main() -> None:
    ...
    document = {"schema": 1, "status": "incomplete", "candidate": "component-private-probe",
                "revision": revision, "host": platform.platform(), "samples": records,
                "encoder_sha256": hashlib.sha256(encoder.read_bytes()).hexdigest(),
                "cache_mode": "soldr-disabled", "cache_hit_rate": None,
                "peak_compiler_rss_bytes": None,
                "limitations": ["shared host, not the controlled reference-host latency gate",
    ...
    for index in range(args.edits + 2):
        ...
        encode_ns, result = run([str(encoder), str(artifact), str(component)], source,
