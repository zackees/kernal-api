//! Serializable cache fingerprint compatibility, not an immutable file handle.
//! Metadata and identity observations may race path replacement. Do not use this
//! cache key as authorization or proof of an unchanged open file.

use serde::{Deserialize, Serialize};
use std::path::Path;

/// Serializable metadata fingerprint for cache invalidation.
///
...
/// compare equal only when every populated member matches. This is a snapshot,
/// not an immutable identity: timestamps can change and paths can be replaced.
...
#[cfg(unix)]
pub fn same_file_by_fingerprint(a: &Path, b: &Path) -> bool {
    match (file_fingerprint(a), file_fingerprint(b)) {
